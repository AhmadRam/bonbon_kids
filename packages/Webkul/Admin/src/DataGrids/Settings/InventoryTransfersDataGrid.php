...
class InventoryTransfersDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder()
    {
        ...
        $queryBuilder = DB::table('inventory_transfers')
            ...
            ->addSelect('inventory_transfers.id as id', 'product_flat.product_number as product_number', 'product_flat.name as product_name', 'product_flat.sku as product_sku', 'from.name as from_name', 'to.name as to_name', 'inventory_transfers.quantity as quantity');
    ...
    public function prepareColumns()
    {
        ...
        $this->addColumn([
            ...
            'closure' => function ($row) {
                return $row->from_name === 'Default' ? 'Hawalli' : $row->from_name;
            },
        ]);
        ...
        $this->addColumn([
            ...
            'sortable' => true,
            'closure' => function ($row) {
                return $row->to_name === 'Default' ? 'Hawalli' : $row->to_name;
            },
        ]);
    ...
    public function prepareActions()
    {
        $this->addAction([
            'icon' => 'icon-view',
            'title' => trans('admin::app.sales.invoices.view.print'),
            'method' => 'GET',
            'url' => function ($row) {
                return route('admin.settings.inventory_sources.print_transfer', $row->id);
            },
        ]);
    }
    ...
    {
        // Add dummy mass action to enable checkboxes
        $this->addMassAction([
            'icon' => 'icon-delete',
            'title' => 'Delete (Not working - Use Print button above)',
            'method' => 'POST',
            'url' => '#',
        ]);
    }
